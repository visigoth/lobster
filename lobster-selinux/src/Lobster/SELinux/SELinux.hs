...
prettyPrintSELinux selinux = render $
    Pp.above ([ text "--- Type enforcement (.te) file ---"
             , Pp.pp (typeEnforcement selinux)
             , text "--- File context (.fc) file ---"
             ] ++ map Pp.pp (fileContext selinux))
