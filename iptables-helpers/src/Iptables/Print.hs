...
printIptables (Iptables f n m r s) =
    ...
    ++ (if null s then "" else printTable "security" s)
    where
...
printTarget rt = (++) "-j " $ case rt of
                            ...
                            TSecmark ctx        -> "SECMARK --selctx " ++ ctx
                            TConnsecmarkRestore -> "CONNSECMARK --restore"
                            TConnsecmarkSave    -> "CONNSECMARK --save"
                            TUnknown tName opts ->
                                tName ++ " " ++ unwords opts
...
printMacAddress (MacAddr a b c d e f) = showHex2 a $ showChar ':'
    ...
    where
        ...
        showHex2 w = if w < 16 then showChar '0' . showHex w
                               else showHex w
