...
module Iptables.Parser (parseIptables, ParseError) where
...
parseIptables = runParser iptables [] "input" . removeComments
    ...
    iptables = do
        ...
        let securityL = filter (isTable "security") tables
        let security = headDef [] $ map snd securityL
        return $ Iptables filter' nat mangle raw security
    ...
    table = do
        ...
        when (tableName /= "filter" && tableName /= "nat" && tableName /= "mangle" && tableName /= "raw" && tableName /= "security") $
    ...
    ruleTarget =
        ...
        tSecmark = do
            ...
            string "--selctx"
            char ' '
            ctx <- many $ satisfy (not . isSpace)
            return $ TSecmark ctx

        tConnsecmarkRestore = do
            try $ string "CONNSECMARK --restore"
            return TConnsecmarkRestore

        tConnsecmarkSave = do
            try $ string "CONNSECMARK --save"
            return TConnsecmarkSave
