...
isRawBuiltinChain :: String -> Bool
isRawBuiltinChain chain =
    case chain of
        "PREROUTING" -> True
        "OUTPUT" -> True
        _ -> False

isSecurityBuiltinChain :: String -> Bool
isSecurityBuiltinChain chain =
    case chain of
        "INPUT" -> True
        "FORWARD" -> True
        "OUTPUT" -> True
...
-- | @isBuiltinChain table chain@ returns @True@ for built-in chains and @False@ otherwise.
isBuiltinChain :: String -> String -> Bool
isBuiltinChain table chain =
    case table of
      "filter" -> isFilterBuiltinChain chain
      "nat" -> isNatBuiltinChain chain
      "mangle" -> isMangleBuiltinChain chain
      "raw" -> isRawBuiltinChain chain
      "security" -> isSecurityBuiltinChain chain
      _ -> False
...
sortIptables (Iptables filter nat mangle raw security) = Iptables (sortFilterTable filter)
                                                           (sortNatTable nat)
